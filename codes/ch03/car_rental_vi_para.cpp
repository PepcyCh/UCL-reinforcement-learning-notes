...
/*
 * general codes for value iteration
 */
...
/*
 * Value Iteration (parallel)
 *
 * Routine
 *   Loop util values are kept unchanged (epsilon-converge):
 *     Calc Bellman optimality equation
 *
 * Time complexity
 *   O(n^2 m) for each step of value iteration
 *   n - # states
 *   m - # actions
 *
...
class ValueIteration {
  public:
    void Train() {
        ...
        while (true) {
            ...
            LOG("BOE time used: " << time_used.count() << "s");

            LOG("After " << iter << " iterations, difference of values is " <<
                    diff);
            if (diff < eps)
                break;
        }
        ...
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> time_used = end - start;
        LOG("Getting policies used: " << time_used.count() << "s");
  ...
  protected:
    ...
    std::pair<Reward, State> CalcOptimality(const State &state) const {
        ...
        return std::make_pair(max, state);
    }

    Policies GetPoliciesFromValues() const {
        Policies new_policies;
        std::vector<std::future<std::pair<Policy, State>>> handles;
        for (const auto &state : all_states) {
            handles.push_back(std::async(std::launch::async,
                    [this, &state]() { return this->DoGreedy(state); }));
        }
        for (auto &future : handles) {
    ...
    std::pair<Policy, State> DoGreedy(const State &state) const {
        Reward max = std::numeric_limits<Reward>::lowest();
        int n_max = 0;
...
class CarRental : public ValueIteration<CarRentalState, CarRentalState::Action,
